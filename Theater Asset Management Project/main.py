...
def update_detail(user_details):
    ...
    choice = input("Which detail would you like to modify? (name/location/projector/screen/sound/seats/status):\n ").lower()
    if choice == 'name':
...
def add_details(user_details):
    print("\nAdd new details:")
    detail=int(input("Which detail would you like to add?\n1.parking\n2.food counter:\n"))
    if detail==1:
...
def delete_details(user_details):
    print("\nDelete details")
    delete=input("Which detail would you like to delete?\n(name/location/projector/screen/sound/seats/status/cleaning/security):\n ").lower()
    if delete=="name":
